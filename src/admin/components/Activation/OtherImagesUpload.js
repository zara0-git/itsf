...
export default function (props) {
  ...
  //   const server = server;
  const ActivationId = props.ActivationId ? props.ActivationId : null;

  let fileInput = React.createRef();

  React.useEffect(() => {
    props.ActivationId && GetData();
  }, [props.ActivationId]);
  ...
  const GetData = async () => {
    var ActivationIdValue = [];
    if (ActivationId) {
      var Response = await server.CallService({
        Method: "GET",
        Url: "activation/image/list/" + ActivationId,
      });
          ...
          ResImages.forEach((image) => {
            ActivationIdValue.push({
              ...
              FileInfo: {
                ...
                Id: image.id,
                Link:
                  server.BaseFileUrl + "ourActivition/" + image.generated_name,
              },
            });
          });

        setValue(ActivationIdValue);
        setUpdate(!update);
      } else {
        setValue([]);
      }
