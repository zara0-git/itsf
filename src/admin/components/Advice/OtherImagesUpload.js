...
export default function (props) {
  ...
  //   const server = server;
  const AdviceId = props.AdviceId ? props.AdviceId : null;

  let fileInput = React.createRef();

  React.useEffect(() => {
    props.AdviceId && GetData();
  }, [props.AdviceId]);
  ...
  const GetData = async () => {
    ...
    if (AdviceId) {
      var Response = await server.CallService({
        Method: "GET",
        Url: "advice/image/list/" + AdviceId,
      });
          ...
          ResImages.forEach((image) => {
            AdviceValue.push({
              ...
              FileInfo: {
                ...
                Type: image.type,
                Id: image.id,
                Link: server.BaseFileUrl + "advice/" + image.generated_name,
              },
            });
          });

        setValue(AdviceValue);
        setUpdate(!update);
      } else {
        setValue([]);
      }
