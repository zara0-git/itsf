...
export default function (props) {
  ...
  //   const server = server;
  const CustomerId = props.CustomerId ? props.CustomerId : null;

  let fileInput = React.createRef();

  React.useEffect(() => {
    props.CustomerId && GetData();
  }, [props.CustomerId]);
  ...
  const GetData = async () => {
    ...
    if (CustomerId) {
      var Response = await server.CallService({
        Method: "GET",
        Url: "customers/image/list/" + CustomerId,
      });
          ...
          ResImages.forEach((image) => {
            CustomerValue.push({
              ...
              FileInfo: {
                ...
                Type: image.type,
                Id: image.id,
                Link: server.BaseFileUrl + "customer/" + image.generated_name,
              },
            });
          });
